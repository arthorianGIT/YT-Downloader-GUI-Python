...
async def download_video(url, video_ext_value, video_scale_value, directory):
    ydl_opts = {
...
async def download_audio(url, audio_ext_value, audio_kbps_value, directory):
    ydl_opts = {
        'format': 'bestaudio',
        'extractaudio': True,
...
async def download_both(url, video_ext_value, video_scale_value, audio_ext_value, audio_kbps_value, directory):
    ydl_opts = {
        ...
        'quiet': False,
        'outtmpl': f'{directory}/%(uploader)s - %(title)s'
    }
    await asyncio.to_thread(yt_dlp.YoutubeDL(ydl_opts).download, [url])